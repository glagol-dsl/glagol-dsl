...
test bool shouldTransformToFetchLocalPropPhpExpr()
    = toPhpExpr(get(artifact(unresolvedName("SomeUtil"))))
...
test bool shouldTrasnformToNew() = toPhpExpr(new(unresolvedName("MyUtil"), [])) == phpNew(phpName(phpName("MyUtil")), []);
test bool shouldTrasnformToNewWithArguments()
    = toPhpExpr(new(unresolvedName("MyUtil"), [variable("bla")])) == phpNew(phpName(phpName("MyUtil")), [phpActualParameter(phpVar(phpName(phpName("bla"))), false)]);
