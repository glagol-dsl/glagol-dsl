...
alias Code = list[tuple[str line, loc origin, bool useEnd]];
...
public Code glue(list[lrel[str, loc, bool]] pieces, Code s) = code("") when size(pieces) == 0;
public Code glue(list[lrel[str, loc, bool]] pieces, Code s) = pieces[0] when size(pieces) == 1;
public Code glue(list[lrel[str, loc, bool]] pieces) = glue(pieces, code(""));
public default Code glue(list[lrel[str, loc, bool]] pieces, Code d) = (pieces[0] | it + d + p | p <- slice(pieces, 1, size(pieces) - 1));

public str implode(Code pieces) = ("" | it + l | <str l, loc d, bool _> <- pieces);
