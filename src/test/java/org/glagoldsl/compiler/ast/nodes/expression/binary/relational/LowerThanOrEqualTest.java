...
class LowerThanOrEqualTest {
    @Test
    void accept(@Mock ExpressionVisitor<Void, Void> visitor) {
        var node = new LowerThanOrEqual(mock(Expression.class), mock(Expression.class));
        node.accept(visitor, null);
        verify(visitor, times(1)).visitLowerThanOrEqual(any(), any());
    }
}